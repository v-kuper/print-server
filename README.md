...
- печатает только текстовые сообщения, включая пересланный текст; на фото,
  голосовые сообщения, видео и другие медиафайлы отвечает, что они не
  поддерживаются;
- печатает дату и время сообщения в часовом поясе `Europe/Minsk`;
- отвечает отправителю в Telegram: `Факс доставлен.`, если печать прошла,
  или `Факс принят. Принтер сейчас недоступен или занят; распечатаем и уведомим вас.`,
  если сообщение осталось в очереди;
- при успешной допечатке очереди отправляет `Факс доставлен.`, а после
  исчерпания повторных попыток — `Факс не доставлен. Проверьте принтер и отправьте факс ещё раз.`;
