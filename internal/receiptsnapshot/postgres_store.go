...
func (s *PostgresStore) Create(ctx context.Context, input CreateInput) (Snapshot, error) {
	...
	err = s.pool.QueryRow(ctx, `
		INSERT INTO receipt_snapshots (workspace_id, status, news_items, receipt_lines)
		VALUES ($1::uuid, $2, $3::jsonb, $4::jsonb)
		RETURNING id::text, workspace_id::text, status, news_items, receipt_lines, error, created_at, published_at, failed_at`,
		s.workspaceID, StatusPending, string(newsData), string(lineData),
	).Scan(&snapshot.ID, &snapshot.WorkspaceID, &snapshot.Status, &rawItems, &rawLines, &snapshot.Error, &snapshot.CreatedAt, &snapshot.PublishedAt, &snapshot.FailedAt)
	if err != nil {
		return Snapshot{}, err
	}
...
func (s *PostgresStore) Load(ctx context.Context, id string) (Snapshot, error) {
	snapshot, err := s.querySnapshot(ctx, `
		SELECT id::text, workspace_id::text, status, news_items, receipt_lines, error, created_at, published_at, failed_at
