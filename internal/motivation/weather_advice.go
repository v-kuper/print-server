...
func weatherAdvicePrompt(weather WeatherContext, previousAdvice string) string {
	var parts []string
	...
	prompt := "Вот данные погоды на день:\n" +
		strings.Join(parts, "\n") +
		"\n\nНапиши короткий живой комментарий к семейной прогулке на русском языке для печати на чековой ленте. " +
		"Контекст: гулять собираются пользователь, его жена и их собака Бонни — девочка породы джек-рассел. " +
		"Пусть это звучит естественно и разговорно, как близкий человек сказал бы дома после взгляда на погоду: тепло, конкретно и без рекламной поэзии. " +
		"Не разыгрывай диалог, не говори от имени жены и не перечисляй членов семьи в каждом ответе; упоминай Бонни или семью только когда это звучит органично. " +
		"Сначала молча выбери один самый уместный ракурс: комфорт на улице, темп, маршрут, ближайшее изменение погоды или конкретный погодный риск. Не перечисляй все варианты и меняй ракурс от генерации к генерации. " +
		"Опирайся только на эти данные. Не выдумывай события, прогнозы, солнце, дождь, снег или сильный ветер, если их нет в данных. " +
		"Комментарий должен учитывать температуру, осадки, ветер и состояние погоды, если они указаны, и не противоречит погоде. " +
		...
		"Не делай список обязательных вещей и не навязывай предметы без необходимости. Не упоминай детей: их нет во входном контексте. " +
		"Избегай штампов и близких перефразировок: 'променад', 'не спеша', 'погода располагает', 'самое время прогуляться', 'идеально для прогулки'. " +
		"Меняй начало, ритм и структуру фразы. Без markdown, без кавычек, 1-2 короткие строки.\n" +
		generationVariantInstruction()

	previousAdvice = sanitizeQuote(previousAdvice)
	if previousAdvice != "" {
		prompt += "\nПредыдущий погодный комментарий: " + previousAdvice + ". Не повторяй его и не перефразируй близко; выбери другой ракурс и начало."
	}
